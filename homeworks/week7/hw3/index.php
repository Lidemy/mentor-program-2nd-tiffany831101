...
<body>
    ...
    ?>
        ...
        $pages = ceil($page_row['datanum']/$perpage); //無條件進位，因為如果有45筆，就會需要五頁
            ...
            $parent_result = $conn->prepare("SELECT c.id AS cmmt_id, user_id, nickname, time, content FROM $cmmts_table AS c INNER JOIN" . 
					" $users_table ON parent_id = 0 AND user_id = $users_table.id ORDER BY time DESC LIMIT $start, 10");
